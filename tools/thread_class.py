...
"""
@Author  : LIAN
@Project : SampleDetectionV2
@File    : AsyncPoolHandler.py
@Time    : 2021/10/25
@License : (C) Copyright 2021, RoarPanda Corporation.
"""
...
class AsyncPool(object):
    ...
    def __init__(self, maxsize=1, pool_maxsize=None, loop=None):
        ...
        # 协程池
        self._loop, self._loop_thread, self._thread_pool = self.start_loop(loop, pool_maxsize=pool_maxsize)
    ...
    def no_block_submit(self, func, callback=None):
        """
        非阻塞模式
        提交任务到事件循环
    ...
    def no_block_submit_args(self, func, *args, callback=None):
        """
        非阻塞模式
        提交任务到事件循环
    ...
    def block_submit_args(self, func, *args, callback=None):
        """
        阻塞模式
        提交任务到事件循环
        ...
        # 将协程注册一个到运行在线程中的循环，thread_loop 会获得一个环任务
        # 注意：run_coroutine_threadsafe 这个方法只能用在运行在线程中的循环事件使用
        future = asyncio.run_coroutine_threadsafe(self.async_thread_pool_func(func, *args), self._loop)
...
if __name__ == "__main__":
    ...
    """
    使用方式：
    # 实例化类 maxsize： 最大线程池  pool_maxsize 最大协程数
    def main()
        pool = AsyncPool(maxsize=100, pool_maxsize=10)
        # 调用非阻塞方法 
        pool.no_block_submit(async_func,callback=callback_func)
